...
public class Flow {
	...
	private void readFlow() {
        ...
        // get first element in kanter[a]
        // Search linklist le for x, set y2 to old x inverse
				while (le != null) {
					if (le.equals(x)) {
						y2 = le.invers;
						break;
    ...
    for (int i = 0; i <= v; i++) {
      ...
      // Search linklist le for x, set y2 to old x inverse
      while (le != null) {
        System.out.println(le.from + " " + le.to);
        le = le.next;
        count += 1;
      }
    }
